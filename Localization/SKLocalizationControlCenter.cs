    ...
    public class SKLocalizationControlWindow : EditorWindow
    {
        ...
        private static void DrawCenterContent()
        {
            ...
            if (page == CurrentPageSelection.Text)
            {
                ...
                EditorGUI.LabelField(new Rect(new Vector2(0, 60), new Vector2(1400, 20)), $"Text localizations");
                ...
                for (int i = 0; i < asset.textConfigs.Count; i++)
                {
                    ...
                    asset.textConfigs[i].id = EditorGUILayout.IntField(asset.textConfigs[i].id, GUILayout.Width(50));
                    asset.textConfigs[i].descrip = EditorGUILayout.TextField(asset.textConfigs[i].descrip, GUILayout.Width(150));
                    EditorGUILayout.EndHorizontal();
                    for (int j = 0; j < Enum.GetNames(typeof(LanguageSupport)).Length; j++)
                    {
                        if (asset.languageSupports.Contains((LanguageSupport)j))
                        {
                            ...
                            asset.textConfigs[i].localTexts[j] = EditorGUILayout.TextField(asset.textConfigs[i].localTexts[j], GUILayout.Width(500), GUILayout.Height(20));
                            EditorGUILayout.LabelField($"   Font override: ");
                            asset.textConfigs[i].fontOverrides[j] = EditorGUILayout.IntField(asset.textConfigs[i].fontOverrides[j], GUILayout.Width(50));
                            EditorGUILayout.EndHorizontal();
                        }
                    }
                    EditorGUILayout.BeginHorizontal(GUILayout.MaxWidth(-50));
        ...
        {
            CommonUtils.SaveObjectToJson(new SKLocalizationAssetJson(asset), "SKLocalizationAsset.json");
        }

        private static void LoadJsonAsset()
        {
            asset.UpdateInfo(CommonUtils.LoadObjectFromJson<SKLocalizationAssetJson>("SKLocalizationAsset.json"));
            UpdateSupportedLangDescrip();
        }
